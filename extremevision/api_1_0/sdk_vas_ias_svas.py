...
def deal_with_vas():
    ...
    # 获取到容器id
    cmd = run_sdk_config_GPU + f" {image_name}_test"
    status, contain_id = sdk_subprocess(cmd)
    os.system(f"docker cp {path}/sdk_package/vas/authorization.sh {contain_id}:/usr/local/ev_sdk ")
    cmd = f"docker exec  {contain_id} bash /usr/local/ev_sdk/authorization.sh"
    subprocess.Popen(cmd, shell=True)
    time.sleep(10)
    tmp_dir = image_name.split("/")[-1]
    cmd = f"docker cp {contain_id}:/usr/local/vas/vas_data/log /tmp/{tmp_dir}"
    os.system(cmd)
    cmd = f"cat /tmp/{tmp_dir}/vas.INFO|grep \"ji_init return = 0\""
    status, res_code = sdk_subprocess(cmd)
    if not status:
        return jsonify(errno=RET.ALGOVERSIONERR, errmsg=f'封装VAS失败,{res_code}')
    if int(res_code[-1]) != 0:
        return jsonify(errno=RET.ALGOVERSIONERR, errmsg=f'封装VAS失败,{res_code}')
    os.system(f"docker stop {contain_id}")
    os.system(f"docker rmi {image_name}_test")
    os.system(f"rm -rf /tmp/{tmp_dir}")
    return jsonify(errno=RET.OK, errmsg='封装VAS成功,可以直接调用VAS')
